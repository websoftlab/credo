...
export default class HtmlDriverReact extends HtmlDriverPrototype<ElementType, ReactElement> {
	...
	async toHTML(
		...
		if (this.ssr && api) {
			...
			try {
				if (isPageFound(this.page)) {
					...
					const message =
						this.page.message || ctx.store.translate("system.page.unknownError", "Unknown error");
		...
		if (!this.ssr) {
			...
			if (!banner) {
				banner = ctx.store.translate(
					"system.page.noscriptBanner",
