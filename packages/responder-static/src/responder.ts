...
export default (function responder(credo: CredoJS, name: string, config: StaticCtorConfig = {}) {
	...
	let exclude: Array<string | RegExp | ((path: string) => boolean)> = Array.isArray(excludePath)
		? excludePath.slice()
		: excludePath
		? [excludePath]
		: [];
	...
	async function responder(ctx: Context, data: ReadPathFound) {
		const { path, stats, contentEncoding, removeContentLength, encodingExt } = data;
		...
		ctx.bodyEnd(fs.createReadStream(path), undefined, ctx.type ? undefined : type(path, encodingExt));
	}
