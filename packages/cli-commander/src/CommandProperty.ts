...
export default abstract class CommandProperty {
	...
	protected constructor(options: Property, propertyName: string) {
		const { name, format, required, description = "", message, min = 0, max = 0, set = [], unique } = options;
	...
	add(value: string) {
		if (!this.multiple && this.value.length) {
			throw isName(this) ? newError(opts.onlyOne, this.name) : newError(args.onlyOne);
		}
		if (this.max !== 0 && this.value.length >= this.max) {
			throw isName(this) ? newError(opts.tooMany, this.name) : newError(args.tooMany);
		}
	...
	protected _error(): false | { error: string } {
		...
		if (this.multiple) {
			if (this.min > 0 && value.length < this.min) {
				return {
					error: isName(this) ? format(opts.notEnough, this.name) : format(args.notEnough),
				};
			}
			if (this.required && value.length === 0) {
				return {
					error: this.message || (isName(this) ? format(opts.required, this.name) : format(args.required)),
				};
			}
			return false;
		}
