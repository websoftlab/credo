...
async function writeClientFailure() {
	...
	if(!await exists(file)) {
		await writeFile(file, `export default function failure(err) {
	if(__DEV__) {
...
export async function buildClient(factory: CredoPlugin.Factory) {
	...
	const {options: {clusters}} = factory;
	...
	async function create(options: {mid?: number, bootloader?: CredoPlugin.HandlerOptional} = {}) {
		...
		const addBoot = (bootloader: CredoPlugin.HandlerOptional) => {
			...
			if(!boot.includes(func)) {
				boot.push(func);
				bootloaders.push(`(api) => typeof ${func} === "function" && ${func}(api${options ? `, ${cJs.tool.esc(options)}` : ""})`);
			}
		};
		...
		cJs.append(`const renderPage = typeof ${func}.renderPage === "function" ? ${func}.renderPage : (typeof ${func}.default === "function" ? ${func}.default : ${func});`);
