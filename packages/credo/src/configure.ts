...
export default async function configure(options: BuildConfigureOptions, builderType: BuilderType): Promise<BuildConfigure> {
	let {
		type,
		mode: modeOption,
		isProd: isProdOption,
	...
	const mode = typeof modeOption === "string" ? getMode(modeOption) : (
		typeof isProdOption === "boolean" ? getIsProdMode(isProdOption) : (
			typeof isDevOption === "boolean" ? getIsProdMode(!isDevOption) : getMode(
				process.env.NODE_ENV
			)
		)
	);

	const isDevServer = typeof isDevServerOption === "boolean" ? isDevServerOption : (
		process.env.WEBPACK_IS_DEV_SERVER === 'true'
	);
	...
	const config: BuildConfigure = {
		...
		isClient,
		cwd: cwdPath,
		cwdPath(...args: string[]) { return join(cwdPath, ... args); },
		bundlePath(...args: string[]) { return join(cwdPath, bundle, ...args); },
		...
		async fireHook(name: CredoPlugin.HooksBundleEvent, arg1?: any) {
			switch(name) {
				case "onRollupConfigure": return factory.fireHook(name, arg1, config);
				case "onWebpackConfigure": return factory.fireHook(name, arg1, config);
			}
		},
