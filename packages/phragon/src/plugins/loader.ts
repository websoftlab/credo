...
import type { EStat, PhragonPlugin, PhragonConfig } from "../types";
...
async function ready(
	...
	const { name, root } = sum;
	const pluginPath = dirname(sum.phragonJsonPath);
	const data = await readJsonFile(sum.phragonJsonPath);
	...
	const def: PhragonPlugin.Plugin = {
		...sum,
		pluginPath,
	...
	async function each(
		...
		entry: Record<string, PhragonPlugin.HandlerOptional>,
		home: string
	) {
	...
	// hooks
	const names: PhragonPlugin.HooksEvent[] = [
		"onBuild",
		"onInstall",
		"onWebpackConfigure",
		"onRollupConfigure",
...
async function load(sum: PluginSum) {
	const plugins: PhragonPlugin.Plugin[] = [];
...
async function pluginSum(name: string, version: string, phragonJsonPath: string, root = false): Promise<PluginSum> {
	return {
		name,
		version,
		phragonJsonPath,
...
async function readStat(dep: { name: string; version: string }) {
	...
	let file: string;
	try {
		file = require.resolve(`${name}/phragon.json`);
	} catch (err) {
...
export async function loadPlugin(module: string): Promise<PhragonPlugin.Plugin[]> {
...
export async function loadRootPluginOnly(): Promise<PhragonPlugin.Plugin> {
	const root = await getRoot();
	if (!root) {
...
export async function loadAllPlugins(): Promise<PhragonPlugin.Plugin[]> {
