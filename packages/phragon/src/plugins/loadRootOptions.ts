import type { DaemonSignKill, PhragonConfig, PhragonPlugin } from "../types";
...
async function loadDriver(renderDriver: string): Promise<PhragonPlugin.RenderDriver> {
	...
	if (baseDriver.includes(renderDriver)) {
		renderDriver = `@phragon/render-driver-${renderDriver}`;
	}
...
export default async function loadRootOptions(plugins: PhragonPlugin.Plugin[]): Promise<PhragonPlugin.RootOptions> {
	...
	const data = await readJsonFile(root.phragonJsonPath);
	const opts = data.options || {};
	const names = plugins.map((plugin) => plugin.name);
	const options: PhragonPlugin.RootOptions = {
		ssr: opts.ssr !== false,
		pages: false,
	...
	const cls: PhragonPlugin.RootClusterOptions[] = [];
	...
	if (Array.isArray(clusters) && clusters.length > 0) {
		...
		for (let cluster of clusters) {
			...
			const pco: PhragonPlugin.RootClusterOptions = {
				id,
				pages: false,
				mid: ids.length + 1,
				mode,
