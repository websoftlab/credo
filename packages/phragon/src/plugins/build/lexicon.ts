...
export async function createLexiconOptions(
	opts: any,
	pluginNames: string[]
): Promise<PhragonPlugin.RootLexiconOptions> {
	let ie: string[] = [];
	const lexicon: PhragonPlugin.RootLexiconOptions = {
		languages: [],
		multilingual: false,
	};
	...
	function incExc(variant: "include" | "exclude", item: string | { name: string; type?: "lambda" | "data" | "all" }) {
		if (typeof item === "string") {
			...
			throw newError(`{green ./phragon.json} lexicon.${variant} invalid plugin value ${item}`);
		}
		...
		if (!type) {
			...
			throw newError(`{green ./phragon.json} lexicon.${type} invalid plugin type ${type}`);
		}
	...
	function variant(
		...
		if (opts.languages) {
			if (!Array.isArray(opts.languages)) {
				throw newError(`{green ./phragon.json} lexicon.{yellow languages} options is not defined`);
			}
...
export async function buildLexicon(factory: PhragonPlugin.Factory) {
	...
	if (route) {
		...
		sJs.group("export default function(phragon)", "", (t) => {
			...
			sJs.group(
				`phragon.route.addRoute(new ${sJs.imp("@phragon/server/RouteManager", "RoutePoint")}(`,
				"), 100);",
				() => {
					sJs.append(`pattern,`);
			...
			sJs.group("async function __id(id, data)", "", () => {
				sJs.append(`const r = await ${sJs.tool.keyVar("phragon.services", service.split("."))}(id);`);
			...
			sJs.group("async function __idPg(id, packageName, data)", "", () => {
				sJs.append(
					`const r = await ${sJs.tool.keyVar("phragon.services", service.split("."))}(id, packageName);`
				);
