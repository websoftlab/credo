import {conf, writeJsonFile} from "../utils";
...
import {BundleVersionJson} from "../types";
...
async function setNpmUser(channel: ConfigChannel, logout = false) {
	...
	const data = await prompts([
		{
			...
			name: "password",
			validate: (text) => String(text || "").trim().length > 3 ? true : "Password is required! Please pick a password longer than 3 characters.",
		}, {
			type: "text",
	...
	return new Promise<void>((resolve, reject) => {
		proc.stdout?.on("data", (chunk) => {
			...
			switch(chunk) {
				case "Username:": proc.stdin?.write(channel.user +"\n"); return;
				case "Password:": proc.stdin?.write(data.password +"\n"); return;
			}
...
export default async function release(name: string, scope: string[] = []) {
	const channel = (await conf()).release[name];
	if(!channel) {
	...
	if(!user) {
		...
		const question = await prompts({
			type: "confirm",
			message: format("You are already signed in as {yellow %s}. Want to log out and log in as {cyan %s}?", user, channel.user),
			name: "yes"
		});
		if(!question.yes) {
